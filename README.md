...
预览生产构建要注意：产物里的资源链接带 `/youngke-blog` 前缀（仓库挂在 GitHub Pages 子路径下，见「部署」一节），所以直接 serve `out/` 会 404。模拟子路径即可：
...
`out/` 是纯 HTML/CSS/JS，丢到任意静态托管都行。

仓库已带 GitHub Pages 流水线（`.github/workflows/deploy.yml`），推到 `main` 自动发布。首次部署需在 **Settings → Pages → Source** 选 **GitHub Actions**。

站点地址是 `https://yyl1208.github.io/youngke-blog/` —— 仓库名不是 `yyl1208.github.io`，所以站点挂在子路径下，`next.config.ts` 会在生产构建时自动加上 `basePath`（本地 dev 不带，避免开发时 404）。

要改域名或仓库名，只改 `lib/site.ts` 一处；换自定义域名时把 `basePath` 逻辑去掉即可。
